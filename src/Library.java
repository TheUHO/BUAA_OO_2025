...
public class Library {
    ...
    private LocalDate lastOpenDate; // 上次开馆日期
    ...
    public void openLibrary(LocalDate date) {
        ...
        for (User user : users.values()) {
            user.deductOverdueCredit(lastOpenDate, date);
        }
    ...
    public void borrowBook(String studentId, LibraryReqCmd req) {
        ...
        if (user.checkoutBorrow(isbn)) { // 检查是否可以借书
            ...
            if (book != null) {
                // 借书成功
                user.addBook(book, req.getDate()); // 添加书籍到用户
    ...
    public void returnBook(String studentId, LibraryReqCmd req) {
        ...
        int overdueDays = user.getOverdueDays(bookId, req.getDate());
        Book book = user.returnBook(bookId, req.getDate()); // 用户还书
        if (book != null) {
            // 还书成功
    ...
    // 查询个人信用积分
    public void queryCreditScore(LibraryQcsCmd req) {
        String studentId = req.getStudentId();
        User user = getUser(studentId);
        int creditScore = user.getCreditScore();
        PRINTER.info(req, creditScore);
    }
    ...
    public void readBook(String studentId, LibraryReqCmd req) {
        ...
        if (!user.checkoutRead(today, req.getBookIsbn())) {
            PRINTER.reject(req);
            return;
        }
    ...
    public void restoreBook(String studentId, LibraryReqCmd req) {
        User user = getUser(studentId);
        Book book = user.restoreReadingBook(req.getBookId(), req.getDate());
        if (book != null) {
            // 从阅读室取回
    ...
    private void clearAppointmentOffice(ArrayList<LibraryMoveInfo> moveInfos, 
        ...
        ArrayList<ReservationInfo> expiredInfos =
            appointmentOffice.getExpiredReservationInfos(date, limitDays);
        for (ReservationInfo info : expiredInfos) {
            getUser(info.getStudentId()).punishNotPickedReservation();
        }
    ...
    private void clearReadingRoom(ArrayList<LibraryMoveInfo> moveInfos, LocalDate date) {
        ...
        for (Book book : bookList) {
            ...
            String reader = readingRoom.getReader(book.getBookId());
            if (reader != null) {
                // 扣除用户信用分
                getUser(reader).punishNotReturnedReading();
                // 清除用户阅读记录
                getUser(reader).clearReading();
            }
        }

...
public class SinFactor implements Factor {
    ...
    public Poly toDerivative() {
        ...
        BigInteger coe = sign.multiply(exponent); // 求导后的系数
        Mono mono = new Mono(coe, BigInteger.ZERO);
        mono.addCosFactor(factor.toPoly(), BigInteger.ONE);
        if (exponent.compareTo(BigInteger.ONE) > 0) { // 如果指数大于1，继续添加余弦因子
            mono.addSinFactor(factor.toPoly(), exponent.subtract(BigInteger.ONE)); 
        }
        result.addMono(mono);
